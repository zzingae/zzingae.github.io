...
또한, 원하는 분포의 값을 normalize 상수  전까지 계산할 수 있다해도 샘플링이 가능하다:

$$
p(z)=\frac{1}{Z_p}\tilde{p}(z)
$$

즉, 어떠한 $z$ 에 대해서도 $\tilde{p}(z)$ 만 계산할 수 있으면 rejection sampling 을 사용할 수 있다.
...
3. 만약 $u_0$ <= $\tilde{p}(z_0)$ 이면 $(z_0,u_0)$ 저장, 아니면 버림
...
위 그림의 회색 부분 $u_0$ > $\tilde{p}(z_0)$ 의 $(z_0, u_0)$ 쌍은 버려지며, 저장된 $(z_0,u_0)$ 는 $\tilde{p}(z)$ 그래프 아래에서 균일하게 분포한다. 따라서, 알고리즘으로 생성된 $z_0$ 들은 원하는 $p(z)$ 분포를 따라 생성된다. 왜냐하면 $p(z)$ 가 높은 $z$ 일수록 저장될 가능성이 커서, 더 많이 저장되기 때문이다 (엄밀한 증명은 Ref. 1, Exercise 11.6 참고).

이 알고리즘이 가능한 이유는 $p$ 에서 샘플 추출은 어려울지라도, 특정 $z_0$ 에서 $\tilde{p}(z_0)$ 을 계산하는것은 가능하기 때문이다.
