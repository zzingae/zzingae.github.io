...
따라서, 샘플링은 target (전체 집단) 분포 $p$ 로 부터, (대표적인) 샘플 $z_1,z_2,..,z_N$ 을 생성하는것이다:
...
target 분포 $p$ 의 샘플 $x \sim p(x)$ 을 얻는 방식이다. 앞서 봤던 GAN이 이 방법으로 이미지를 생성한다. 
...
또한, target 분포 $p$ 를 normalize 하기 전인 $\tilde{p}$ 까지만 안다고 해도 ($Z_p$ 를 계산할 수 없어서), 샘플링이 가능하다:
...
위 그림의 회색 부분 $\tilde{p}(z_0) < u_0$ 의 $(z_0, u_0)$ 쌍은 버려지며, 저장된 $(z_0,u_0)$ 는 $\tilde{p}(z)$ 그래프 아래에서 균일하게 분포한다. 따라서, 알고리즘으로 생성된 $z_0$ 들은 target 분포 $p(z)$ 를 따라 생성된다. 왜냐하면 $p(z)$ 가 큰 $z$ 일수록 저장될 가능성이 커서, 더 많이 저장되기 때문이다 (증명은 Ref. 1, Exercise 11.6 참고).
...
의문점은 이런식으로 얻은 샘플 $z \sim q(z|z_t)$ 들은 target 분포 $p$ 에서 얻은 샘플이 아니라는 것이다. 그러나 Markov chain이 target 분포 $p$ 에 대해 특정 조건들을 만족하면, 얼마 후 $p$ 를 따르게 된다.
...
Metropolis algorithm 의 Markov chain 을 따라 target 분포 $p$ 의 샘플을 추출하려는 동선. 검은색 타원은 target 분포 $p(z) = N(\mu,\Sigma)$ 의 one standard-deviation contour 이다.
...
그림에서 보듯 $z^{\star}$ 가 target 분포 $p$ 의 contour를 벗어나면 reject (빨간색 선) 되는 경우가 많은데, 이는 $p(z^{\star}) < p(z_t)$ 이기 때문에 accept 확률 $A$ 가 작아지기 때문이다.
...
마지막으로 소개할 MCMC 방법은 Gibbs sampling 으로, 이전에 소개했던 방법들에 비해 간단하며 직관적이다. 그리고 Metropolis-Hastings 알고리즘의 특수한 경우라고 해석할 수 있다.

target 분포 $p$ 가 여러개의 변수들 $z_1,z_2,..z_M$ 에 따른 확률을 가지며, joint 분포 $p(z_1,z_2,..z_M)$ 에서 샘플을 직접 얻기 힘들때 사용할 수 있다. Gibbs sampling 은 한번에 하나의 변수 $z_i$ 만 샘플링 하는데, 이때 $z_i$ 를 제외한 나머지 변수들 $z_{/i}\equiv(z_1,..z_{i-1},z_{i+1},..,z_M)$ 을 조건부 확률로 샘플링 한다:

$$
z_i^{(t+1)} \sim p(z_i \mid z_{/i}^{(t)})
$$

새롭게 샘플링된 $z_i^{(t+1)}$ 는 기존의 $z_i^{(t)}$ 를 대체하여 $z_{/i}^{(t)}$ 와 함께 새로운 샘플을 생성한다:

$$
z_1^{(t)},..,z_{i-1}^{(t)},z_i^{(t+1)},z_{i+1}^{(t)},..,z_M^{(t)}
$$

이러한 방식으로 각 변수들을 샘플링하여 기존 변수값을 대체하는 방식으로 Gibbs sampling 이 이뤄진다.
...
Gibbs sampling 은 따로 proposal distribution $q$ 를 사용하지 않고 target 분포의 조건부 확률을 전이확률로 사용한다. 이때, target 분포 $p$ 가 Gibbs sampling 의 Markov chain 에 대해 invariant 분포임을 보일 수 있다:
...
$z^{(t)}$ 가 target 분포를 따를 때, $z_i^{(t+1)}$ 변수만 새로 샘플링하여 $z^{(t+1)}$ 을 생성해도 동일하게 target 분포를 따른다는 것을 확인할 수 있다. 

Gibbs 샘플들이 target 분포를 따르기 위해서는, 각 변수에 대한 조건부 확률들이 모든 위치에서 0 보다 커야 한다는 추가적인 조건이 충족되어야 한다. 조건이 충족되지 않은 경우 왜 target 분포를 따르지 않는지 간단한 예를 보자.
