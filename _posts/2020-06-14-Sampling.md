...
$z^{(t)}$ 가 target 분포를 따를 때, $z_i^{(t+1)}$ 변수만 새로 샘플링하여 $z^{(t+1)}$ 을 생성하면, 동일하게 target 분포를 따른다는 것을 확인할 수 있다. 

Gibbs 샘플들이 초기 분포와 상관없이 target 분포를 따르기 위해서는, 각 변수에 대한 `조건부 확률들이 모든 위치에서 0 보다 커야 한다` 는 충분조건이 있다. 조건이 충족되지 않은 경우 target 분포를 따르지 않는 간단한 케이스를 보자.
