...
위 그림에서 가능한 상태 $S={s_1,s_2,s_3}$ 는 총 세 개 이며, 초기 상태 $p_0(z)$ 확률 분포 그리고 전이 확률 $P$ 이 주어졌다. 전이 확률은 Metropolis algorithm 에서 봤던 proposal distribution $q$ 와 같은 개념이다.

그렇다면 $n$ 번 이동을 한 후의 확률 분포 $p_n(z)$ 는 어떻게 될까? 

$P_n=P^{n} \times P_0=(0,0,1)$

에 점점 가까워진다. 즉, 특정 분포 (stationary distribution) 에 수렴 한다. 또한, 다른 초기 상태 $p_0(z)$ 에서 시작한다고 하더라도 동일한 stationary 분포에 수렴하는 것을 확인할 수 있다. 
...
위 그림의 가능한 상태 $S$ 은 두개이며 초기 확률 분포 $p_0(z)$ 가 주어졌을 때, 특정 분포 $p^{\star}=[0.3636.., 0.6363..]$ 로 수렴하는 것을 확인할 수 있다. 또한, 초기 확률 분포와 관계없이 동일한 $p^{\star}$ 로 수렴하는 것을 계산해볼 수 있다.
...
$p^{\star}$ 가 invariant 분포가 아니라면, 전이 확률 $T$ 에 의해 $z$ 의 분포가 변할 것이다. 
