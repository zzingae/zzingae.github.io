...
위 그림의 회색 부분 $u_0$ > $\tilde{p}(z_0)$ 의 $(z_0, u_0)$ 쌍은 버려지며, 저장된 $(z_0,u_0)$ 는 $\tilde{p}(z)$ 그래프 아래에서 균일하게 분포한다. 따라서, 알고리즘으로 생성된 $z_0$ 들은 원하는 $p(z)$ 분포를 따라 생성된다. 왜냐하면 $p(z)$ 가 높은 $z$ 일수록 저장될 가능성이 크기 때문이다 (엄밀한 증명은 Ref. 1 Exercise 11.6 참고).
