...
Metropolis algorithm 는 MCMC 기법중 하나이다. proposal distribution에서 '후보' 샘플 $z^{\star} ~ q(z^{\star}|z_t)$을 추출한다. $z^{\star}$ 는 다음의 기준에 따라 샘플로써 저장될수도 있고 버려질수도 있다.

수식

수식을 자세히 보면 새롭게 샘플링된 값 $z^{\star}$ 에서의 desire distribution 값 $p(z^{\star})$ 이 클수록 저장 될 가능성이 커진다 ($p(z^{\star})>p(z_t)$ 이면 100% 저장된다). 직관적으로, 원하는 분포의 확률이 큰 곳의 샘플을 저장하는것이 자연스럽다.
...
<p align="center"> 
<img src="../images/discrete-states-hole.png" alt="drawing" width="400"/> 
<center>Ref. </center>
</p>

위 그림에서 상태 공간 $S={s_1,s_2,s_3}$ 는 총 세 개 이며, 초기 상태 $p_0$ 확률 그리고 전이 확률 $P$ 이 주어진다. 전이 확률은 Metropolis algorithm 에서 봤던 $q(z|z_t)$ 와 유사한 개념이다.

그렇다면 $n$ 번 이동을 한 후의 확률 분포 $p_n$ 는 어떻게 될까? 

$P_n=P^{n} \times P_0=(0,0,1)$

에 점점 가까워진다. 즉, 특정 분포에 '수렴'한다. 또한, 다른 초기 상태 $P_0$ 에서 시작한다고 하더라도 동일한 분포에 수렴한다. 

사실 이 간단한 예에서 s3 상태는 머물 확률이 1이기 때문에, 어떤 상태에서 출발해도 s3에 갇히게 된다. 따라서, 시간이 지나면 $p(s_3)=1$ 이 될 수 밖에 없다.

<p align="center"> 
<img src="../images/discrete-states-num.png" alt="drawing" width="400"/> 
<center>Ref. </center>
</p>

위 그림의 상태 공간 $S$ 은 두개이며, 위 예와 같은 블랙홀은 없다. 초기 확률 분포 $p_0$ 가 주어졌을 때, 특정 분포 $p^{\star}=[0.3636.., 0.6363..]$ 로 수렴하는 것을 확인할 수 있으며, 사실 초기 확률 분포와 관계없이 동일한 $p^{\star}$ 로 수렴했다.

따라서 Markov chain 이 원하는 분포로 수렴하도록 설계하면, 원하는 샘플들을 얻을 수 있을 것이다. 그렇다면 어떤 조건일때, 초기 확률 분포와 무관하게 특정 분포로 수렴하게 될까?

### Markov chain in general

Markov chain 은
- 초기 확률 분포: $p_0$
- 전이 확률 분포: $T(z_t,z_{t+1}) \equiv p(z_{t+1}|z_t)$

에 따라 정의된다.

만약 모든 $t$ 에서 전이 확률이 동일하다면, 그 Markov chain을 ```homogeneous``` 라 한다. 위 이산 상태의 예에서 전이 확률들은 항상 같았으므로, homogeneous Markov chain 이였다.

만약 Markov chain 의 이동에도 $p^{\star}$ 의 분포가 바뀌지 않는다면, $p^{\star}$ 는 그 Markov chain에 대해 ```invariant``` (stationary) 분포라고 한다.

$p^{\star}(\mathbf{z})=\sum_{\mathbf{z}^{\prime}} T\left(\mathbf{z}^{\prime}, \mathbf{z}\right) p^{\star}\left(\mathbf{z}^{\prime}\right)$

위 식에서 $T$ 와 $p^{\star}$ 의 식이 다르기 때문에 trivial 하지 않다.

### 수렴 조건

원하는 분포가 invariant 분포 이기 위한 Markov chain 의 충분 조건은, Markov chain의 전이 확률이 ```detailed balance``` 를 만족하는 것이다:
...
위의 예에서 전이 확률 $T$ 가 (원하는) 분포 $p^{\star}$ 에 대해 detailed balance를 만족함을 계산할수 있다. 

$p^{\star}(E)T(E,A)=p^{\star}(A)T(A,E)=0.3636..\times0.7=0.6363..\times0.4$

따라서, Markov chain이 $p^{\star}$ 로 수렴했다. detailed balance는 각 상태에서 전이되는 확률이 동일함을 뜻한다. 즉, 전이되는 확률이 균형 (balance) 을 이뤄, 분포가 변하지 않는다 (invariant).

우리는 Markov chain을 이용하여 원하는 분포에서 샘플들을 얻는 것이다. 따라서, 원하는 분포가 우리가 설계한 Markov chain에 대해 invariant 분포이면 된다. 또한, Markov chain에 의해 초기 분포가 원하는 분포로 도달해야 할 것이다.

초기 분포에 상관없이, invariant 한 원하는 분포로 도달하는 특성을 ```ergodicity``` 라고 하며, ergodic Markov chain은 하나의 unique한 분포로 수렴한다.

homogeneous Markov chain은 몇몇 restriction 하에서 ergodic 임을 보일 수 있다.

주어진 원하는 분포 $P$ 에 대해, Markov chain의 전이 확률이 detailed balance를 만족하도록 설정한다:
1. ```under homogeneous and some regularity```, 초기 확률과 관계없이 unique한 stationary 분포 $d$ 가 존재한다.
2. $P$ 는 detailed balance에 의해 invariant 하다.
3. 따라서, 만약 초기 확률이 $P$ 였다면, Markov chain은 $P$ 로 수렴할 것이다.
4. 1에서 초기 확률에 관계없이 unique한 분포로 수렴하므로, $d=P$ 이다.

따라서, ```homogeneous``` Markov chain 은 전이 확률이 ```detailed balance``` 를 만족하면 몇몇 조건 하에 원하는 분포 $P$ 로 수렴한다.

### Metropolis algorithm (revisit)

전이 확률 proposal distribution 이 symmetric 이기 때문에, detailed balance를 만족한다. 

증명

또한, 전이 확률이 매 스텝 동일하기 때문에 원하는 분포 $P$ 로 수렴한다.

하지만, 한가지 생각할 점은 인접한 샘플들끼리는 독립이 아니라는 것이다. 따라서, 일정 간격을 두고 sub-sampling 하여야 어느정도 독립적인 샘플들을 얻을 수 있다.
