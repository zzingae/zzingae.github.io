...
만약 모든 $t$ 에서 전이 확률이 동일하다면, 그 Markov chain을 ```homogeneous``` 라 한다. 위 이산상태의 예들에서 전이 확률은 항상 같았으므로, homogeneous Markov chain 이였다.

만약 어떤 특정한 Markov chain 의 이동에도 $p^{\star}$ 의 분포가 바뀌지 않는다면, $p^{\star}$ 는 그 Markov chain에 대해 ```invariant``` (stationary) 분포라고 한다.
...
1. ```under homogeneous and some regularity```, 초기 확률과 관계없이 unique한 invariant (stationary) 분포 $d$ 가 존재한다.
